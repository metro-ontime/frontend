...
class DataParser extends Component {
  ...
  componentDidMount() {
    ...
    DataFrame.fromCSV(
      `https://s3-us-west-1.amazonaws.com/h4la-metro-performance/data/vehicle_tracking/processed/${
        this.props.line
      }_lametro-rail/2019-01-30.csv`,
    ).then((df) => {
      ...
      const collectTrip = (trip) => {
        const theTrip = trip.group
          .toCollection()
          .map(row => [row.relative_position, new Date(row.datetime).getTime()]);
        return { data: theTrip };
      };
